...
class VMeter(object):
    ...
    def __init__(self, input_device=None, output_device=None):
        pypm.Terminate() #restart pypm--so that users can try to reattach a vMeter if it was disconnected
        pypm.Initialize()
    ...
    def read(self):
        if self._in.Poll():
            ...
            #print self._in.Read(50)[0]
            return self._in.Read(50)[0][0] #read up to 50 events--only return the most recent one
        return None
    def read_multiple_events(self):
        if self._in.Poll():
            #return self._in.Read(1)[0][0]
            #print self._in.Read(50)
            return self._in.Read(500) #read up to 50 events and return them all
        return None
