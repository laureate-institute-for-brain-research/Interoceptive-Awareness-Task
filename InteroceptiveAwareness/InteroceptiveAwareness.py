...
#def run_try(SID, raID, scan, resk, run_num='Practice'):
    ...
    #g.prefix = 'IA-' + g.session_params['SID'] + '-Admin_' + g.session_params['raID'] + '-run_' + g.run_params['run'] + '-' + start_time 
    ...
    #    StimToolLib.run_instructions(os.path.join(os.path.dirname(__file__), 'media', 'instructions', 'IA_instruct_scheduleP.csv'), g)
    #else:
    #    StimToolLib.run_instructions(os.path.join(os.path.dirname(__file__), 'media', 'instructions', 'IA_instruct_scheduleS.csv'), g)
    
    
    #StimToolLib.show_title(g.win, g.title)
    #StimToolLib.show_instructions(g.win, g.instructions)
    g.trial = 0
    #g.ideal_trial_start = 0
